...
const Cards = () => {
    ...
    return (
        <div className=' flex mt-5 flex-wrap md:justify-start justify-around min-h-full'>
            {loading ? <div className=' flex justify-center items-center w-full h-screen'><ThreeCircles height={40} color='white' /></div> :
                movies.map((movie, id) => (
                    <div key={id} className=' min-h-screen md:w-36 mx-2 shadow-xl drop-shadow-xl cursor-pointer hover:scale-110 hover:-translate-y-1 transition-all ease-in-out rounded my-4 duration-500'>
